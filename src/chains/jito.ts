...
export function createJitoBundle(input: {
  ...
  return {
    state: "jito-bundle-draft",
    profileId: input.profile.id,
    transactionsBase64,
    ...(expectedSignatures ? { expectedSignatures } : {}),
    tip: input.tip,
    bindingHash: hashBundle(transactionsBase64, input.tip),
  };
}

export async function submitJitoBundle(relay: JitoRelayLike, draft: JitoBundleDraft): Promise<JitoBundleSubmitted> {
  const raw = await relay.request<unknown>("sendBundle", [[...draft.transactionsBase64], { encoding: "base64" }]);
  const bundleId = resultValue(raw);
...
export async function readJitoBundleStatus(relay: JitoRelayLike, submitted: JitoBundleSubmitted): Promise<JitoBundleStatusEvidence> {
  ...
  const transactionsRaw = Array.isArray(record.transactions) ? record.transactions : [];
  const transactions = transactionsRaw.map((value, index) => {
  ...
  if (submitted.expectedSignatures) {
    const expected = submitted.expectedSignatures.map((signature) => signature.toString());
    const actual = transactions.map((signature) => signature.toString());
  ...
  return {
    state: "jito-bundle-status",
    bundleId: submitted.bundleId,
    found: true,
    ...(record.slot !== undefined ? { slot: integer(record.slot, "slot") } : {}),
    ...(confirmationStatus ? { confirmationStatus: confirmationStatus as SolanaCommitment } : {}),
    transactions,
...
export function assertJitoBundleFinalized(status: JitoBundleStatusEvidence): JitoBundleStatusEvidence & { readonly found: true; readonly confirmationStatus: "finalized" } {
  if (!status.found) fail("ES4478", "JitoBundleNotFound", "Jito bundle has not been observed on-chain.", { bundleId: status.bundleId });
